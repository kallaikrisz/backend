...
class Orszag {
  ...
  static async getAll() {
    try {
      const [rows] = await userPool.query('SELECT * FROM orszagok');
      return rows;
    } catch (err) {
  ...
  static async getByRegion(regio) {
    try {
      const [rows] = await userPool.query('SELECT * FROM orszagok WHERE regio = ?', [regio]);
      return rows;
    } catch (err) {
      throw err;
    }
  ...
  static async getById(id) {
    try {
      const [rows] = await userPool.query('SELECT * FROM orszagok WHERE id = ?', [id]);
      return rows;
    } catch (err) {
      throw err;
    }
  ...
  static async filterOrszagok({ regio, kodReszlet }) {
    ...
    const [rows] = await userPool.query(sql, params);
    return rows;
  }
...
static async getPagedOrszagok({ page = 1, limit = 10, sortBy = 'id', order = 'asc' }) {
  ...
  const sql = `
    ...
    LIMIT ? OFFSET ?
  `;
  const [rows] = await userPool.query(sql, [Number(limit), Number(offset)]);

  // Összes rekord megszámolása, hogy vissza lehessen adni a lapozás információit
  const [countRows] = await userPool.query('SELECT COUNT(*) AS total FROM orszagok');
